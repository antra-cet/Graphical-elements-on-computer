...
void Lab4::RenderScene() {
    ...
    modelMatrix *= transform3D::Translate(translateX * 0.01, translateY * 0.01, translateZ * 0.01);
...
void Lab4::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_L)) {
        miniViewportArea.x += deltaTime*50;
    }

    if (window->KeyHold(GLFW_KEY_K)) {
        miniViewportArea.y -= deltaTime * 50;
    }
    ...
    if (window->KeyHold(GLFW_KEY_U)) {
       miniViewportArea.height += deltaTime * 50;
       miniViewportArea.width += deltaTime * 50;
